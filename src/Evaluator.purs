...
data EvalError =
  ...
  | NoMatchingFunction String [MatchingError]
  | CannotEvaluate Expr
...
--  (List (e:es))                      -> return $ Binary Cons e (List es)
  ...
  (App (Lambda binds body) args)     -> tryAll [Tuple binds body] args "lambda" []
...
binary = go
  ...
  go Div (Atom (Num i)) (Atom (Num 0)) = throwError DivByZero
  go Div (Atom (Num i)) (Atom (Num j)) = retA $ Num $ Math.floor (i / j)
  go Mod (Atom (Num i)) (Atom (Num 0)) = throwError DivByZero
...
apply env name args = case lookup name env of
  ...
  Just cases -> tryAll cases args name []

tryAll :: [Tuple [Binding] Expr] -> [Expr] -> String -> [MatchingError] -> Evaluator Expr
tryAll []                         _    name errs = throwError $ NoMatchingFunction name errs
tryAll (Tuple binds body : cases) args name errs | (length args) < (length binds) = tryAll cases args name (errs ++ [TooFewArguments binds args])
tryAll (Tuple binds body : cases) args name errs = case runMatcherM $ matchls' binds args of
  Right replMap                 -> replace' replMap body >>= wrapLambda binds args
  Left se@(StrictnessError _ _) -> throwError $ NoMatchingFunction name (errs ++ [se])
  Left err                      -> tryAll cases args name (errs ++ [err])
...
matchls' :: [Binding] -> [Expr] -> Matcher (StrMap Expr)
...
match' (ConsLit b bs)    (List [])           = throwError $ MatchingError (ConsLit b bs) (List [])
